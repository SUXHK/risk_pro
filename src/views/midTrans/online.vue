  ...
  <el-card class="cards" :class="tableParams.full ? 'fullScreen' : ''">
    ...
    <el-row :gutter="0" style="margin-bottom:10px">
      ...
      >
        ...
        >
          <el-form-item label="交易业务类型:" prop="transType">
            <el-select
        ...
        >
          <el-form-item label="交易对方账户名称:" prop="partAccName">
            <el-input
              v-model="queryForm.partAccName"
              placeholder="请输入交易对方账户名称"
              clearable
              :style="{ width: '100%' }"
            >
            </el-input>
        ...
        >
          <el-form-item label="交易对方账户类别:" prop="accFlag">
            <el-select
              v-model="queryForm.accFlag"
              placeholder="请选择交易对方账户类别"
              clearable
              :style="{ width: '100%' }"
            >
              <el-option
                v-for="(item, index) in accFlagOptions"
                :key="index"
                :label="item.label"
                :value="item.value"
                :disabled="item.disabled"
        ...
        >
          <el-form-item label="对手账号标识:" prop="tranFlag">
            <el-select
              v-model="queryForm.tranFlag"
              placeholder="请选择对手账号标识"
              clearable
              :style="{ width: '100%' }"
            >
              <el-option
                v-for="(item, index) in tranFlagOptions"
                :key="index"
                :label="item.label"
                :value="item.value"
                :disabled="item.disabled"
...
// import { midTransOnline } from '@/assets/selectoptions/typeOptions.json'
import { midTransOnline } from '@/assets/selectoptions/midTrans/online'
export default {
  components: {
    TableSetting
  },
  data() {
    return {
      ...
      queryForm: {
        ...
        transType: '', // 交易类型 字符型 2
        lendFlag: '', // 资金收付标识 字符型 2
        partAccName: '', // 交易对方账户名称 字符型 80
        accFlag: '', // 交易对方账户类别 字符型 2
        tranFlag: '' // 对手账号标识
      },
      queryFormRules: {
        joinCode1: [],
      ...
      // 交易类型
      transCurrcyCodeOptions: midTransOnline.transCurrcyCodeOptions,
      // 交易类型
      transTypeOptions: midTransOnline.transTypeOptions,
      // 资金收付标识
      lendFlagOptions: midTransOnline.lendFlagOptions,
      // 交易对方账户类别
      accFlagOptions: midTransOnline.accFlagOptions,
      // 对手账号标识
      tranFlagOptions: midTransOnline.tranFlagOptions
    }
  },
  created() {
    this.bizQuery()
